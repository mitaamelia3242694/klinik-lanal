...
@section('content')
        ...
        <div id="modalTambah" onclick="if(event.target === this) this.style.display='none'"
            ...
            <div
                ...
                <form method="POST" action="{{ route('data-kajian-awal.store') }}">
                    ...
                    <select name="pasien_id" required class="input-style" id="pasienDropdown">
                        <option value="">-- Pilih Pasien --</option>
                        @foreach ($pasiens as $item)
                            <option value="{{ $item->pasien->id }}" data-keluhan="{{ $item->keluhan }}">
                                {{ $item->pasien->nama }} -
                    ...
                    <textarea name="keluhan_utama" rows="2" required class="input-style" id="keluhanUtama"
                        readonly></textarea>
    ...
    <script>
        document.getElementById('pasienDropdown').addEventListener('change', function () {
            const selectedOption = this.options[this.selectedIndex];
            const keluhan = selectedOption.getAttribute('data-keluhan') || '';
            document.getElementById('keluhanUtama').value = keluhan;
        });
    </script>
